...
import subprocess
import uuid
from gtsdb_eval import gtsdb_eval
...
class gtsdb(imdb):
    def __init__(self, image_set, devkit_path):
        ...
        self._classes = ('__background__', # always index 0
                         '(danger)', '(mandatory)', '(other)', '(prohibitory)')
    ...
    def _load_gtsdb_annotation(self, index):
        """
        Load image and bounding boxes info from txt files of gtsdb.
        """
    ...
    def _write_gtsdb_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} results file'.format(cls)
            filename = self._get_gtsdb_results_file_template().format(cls)
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_gtsdb_results_file(all_boxes)
    ...
    def _get_gtsdb_results_file_template(self):
        # gtsdbdevkit/results/comp4-44503_det_test_{%s}.txt
